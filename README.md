...
* Größe im Arbeitsspeicher: ~0,7 MB (~7 MB mit conhost.exe)
...
3. Anlegen einer Verknüpfung per Rechtsklick auf Teefax.exe. Damit das Programm immer die gewünschte Zeit abzählt, muss die Verknüpfung um die Sekundenzahl oder weitere gewünschte Parameter erweitert werden. Dazu klickt man mit der rechten Maustaste auf die Verknüpfung und öffnet die Eigenschaften. Dort ergänzt man im Ziel-Pfad am Ende die Parameter oder Sekundenzahl: `C:\Tools\Teefax\Teefax.exe 180`. Diese Verknüpfung lässt sich auch einfach in der Taskleiste plazieren.
...
- Alarm bei Uhrzeit (basierend auf Systemzeit): `-a HH:mm[:ss]` oder `--at HH:mm[:ss]` zählt bis zur angegebenen Uhrzeit.
- Ewiger Wecker (täglich zu bestimmten Uhrzeiten): `--daily HH:mm[:ss]` oder `-d HH:mm[:ss]`. Beispiel: `teefax -d 4:00 10:00 16:00 22:00`
